...
namespace TakeUpJewel.Util
{
	...
	public static class ResourceUtility
	{
		...
		private static readonly Dictionary<string, MidiData> _musicList = new Dictionary<string, MidiData>();
		public static Dictionary<string, MidiData> MusicList => _musicList;
		...
		public static void Init()
		{
			...
			if (DX.LoadDivGraph("Resources\\Graphics\\spplayer_magic.png", 72, 18, 4, 16, 32, out MagicPlayer[0]) == -1)
			{
			...
			if (DX.LoadDivGraph("Resources\\Graphics\\spModokee.png", 8, 8, 1, 32, 16, out ModokeeGround[0]) == -1)
			{
				throw new Exception("キャラの読み込みに失敗しました。");
			...
			if (DX.LoadDivGraph("Resources\\Graphics\\spCaveModokee.png", 8, 8, 1, 32, 16, out ModokeeCave[0]) == -1)
			{
				throw new Exception("キャラの読み込みに失敗しました。");
			...
			if (DX.LoadDivGraph("Resources\\Graphics\\spitem.png", 14, 14, 1, 16, 16, out Item[0]) == -1)
			{
				throw new Exception("キャラの読み込みに失敗しました。");
			}
	...
	public static class ParticleUtility
	{
		...
		{
			for (var i = 0; i < 13 + DX.GetRand(4); i++)
			{
		...
		{
			for (var i = 0; i < 3 + DX.GetRand(3); i++)
			{
	...
	{
		static int[] _hFont;
		static int[] _hFontMini;
		static readonly Dictionary<char, int> DicFont = new Dictionary<char, int>();
		static bool _isInit;
		public static void Init()
		{
			_hFont = new int[384];
			DX.LoadDivGraph("Resources\\Graphics\\font.png", 384, 16, 24, 10, 10, out _hFont[0]);
			_hFontMini = new int[320];
			DX.LoadDivGraph("Resources\\Graphics\\font_mini.png", 320, 16, 20, 8, 8, out _hFontMini[0]);
		...
		public static void DrawString(int x, int y, string txt, int color)
		{
			...
			for (var i = 0; i < txt.Length; i++)
			{
				...
				if (target > 0)
					DX.DrawGraph(_x, _y, _hFont[target], 1);
				else
		...
		public static void DrawMiniString(int x, int y, string txt, int color)
		{
			...
			for (var i = 0; i < txt.Length; i++)
			{
				...
				else if (txt[i] == ' ' || txt[i] == '　')             //空白文字はないので飛ばす(空白は開く)
				{
					...
					target = 0;
				DX.DrawGraph(_x, _y, _hFontMini[target], 1);
				_x += txt[i] < 128 ? 6 : 8;
			}
			DX.SetDrawBright(255, 255, 255);
		}
	...
	public static class SoundUtility
	{
		...
		public static void PlaySound(Sounds snd)
		{
			if (snd == Sounds.Null)
				return;
			snd--;
			DX.PlaySoundMem(_soundlist[(int)snd], DX.DX_PLAYTYPE_BACK);
		}
		...
		public static void PlaySound(int snd)
		{
			if (snd == -1)
				return;
			DX.PlaySoundMem(_soundlist[snd], DX.DX_PLAYTYPE_BACK);
		}
		...
		public static void PlaySound(Sounds snd, int freq, int volume, int pan)
		{
			if (snd == Sounds.Null)
				return;
			snd--;
			DX.SetFrequencySoundMem(freq, _soundlist[(int)snd]);
			DX.ChangeNextPlayVolumeSoundMem(volume, _soundlist[(int)snd]);
			DX.ChangeNextPlayPanSoundMem(pan, _soundlist[(int)snd]);
			if (DX.PlaySoundMem(_soundlist[(int)snd], DX.DX_PLAYTYPE_BACK) == -1)
			{
				throw new Exception("再生に失敗しました");
			}
			DX.ChangeNextPlayVolumeSoundMem(255, _soundlist[(int)snd]);
			DX.ChangeNextPlayPanSoundMem(0, _soundlist[(int)snd]);
